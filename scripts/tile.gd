...
var tile_type: TileType   # Referência ao Resource (forma + cor + elemento) desta peça
...
func _on_mouse_entered():
	...
	var highlight = Color(base_color.r * 1.2, base_color.g * 1.2, base_color.b * 1.2, base_color.a)
	tween.tween_property($Sprite2D, "modulate", highlight, 0.1) # Brighten a partir da cor do elemento
...
func _on_mouse_exited():
	...
	tween.tween_property($Sprite2D, "modulate", base_color, 0.1)

# Set piece type when initializing (forma + cor do elemento)

func set_tile_type(new_type: TileType):
	
	tile_type = new_type
	type = new_type.get_type_id()
	base_color = new_type.color
	
	$Sprite2D.texture = new_type.shape_texture
